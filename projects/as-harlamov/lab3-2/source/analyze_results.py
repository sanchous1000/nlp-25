...
def plot_perplexity_analysis(results_by_iter: Dict, output_dir: Path):
    ...
    for idx, (max_iter, results) in enumerate(sorted(results_by_iter.items())):
        ...
        ax.plot(
            x, y, marker=marker, linestyle='-', label=f'max_iter={max_iter}',
            linewidth=2.5, markersize=10, color=color,
        )
        ax.plot(x_smooth, y_smooth, '--', linewidth=1.5, color=color, alpha=0.5)
        ax.plot(
            best_n_topics, best_perplexity, marker='*', markersize=20,
    ...
    ax.set_title(
        'Зависимость Perplexity от количества тем для разных max_iter',
...
def plot_iterations_analysis(results_by_iter: Dict, output_dir: Path):
    ...
    for idx, n_topics in enumerate(n_topics_list):
        ...
        if iterations:
            ...
            marker = markers[idx % len(markers)]
            ax.plot(
                iterations, perplexities, marker=marker, linestyle='-',
    ...
    ax.set_ylabel('Perplexity', fontsize=14, fontweight='bold')
    ax.set_title(
        'Зависимость Perplexity от количества итераций для разных количеств тем',
        fontsize=16, fontweight='bold',
    )
...
def create_topic_visualization(results_by_iter: Dict, n_topics: int, max_iter: int, output_dir: Path):
    ...
    plt.suptitle(
        f'Топ-10 ключевых слов для каждой темы (n_topics={n_topics}, max_iter={max_iter})',
...
def generate_report(results_by_iter: Dict, output_dir: Path):
    ...
    if best_overall:
        ...
        report_lines.append(
            f"ПРИМЕРЫ ТЕМ ДЛЯ ОПТИМАЛЬНОЙ КОНФИГУРАЦИИ (max_iter={max_iter_best}, n_topics={n_topics_best}):",
        )
        report_lines.append("-" * 80)
