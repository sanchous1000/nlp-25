...
from term_document_matrix import build_term_document_matrix, build_term_document_matrix_from_texts
...
def find_best_polynomial_degree(x: np.ndarray, y: np.ndarray, max_degree: int = 5, min_r2_improvement: float = 0.01) -> Tuple[int, np.ndarray, float]:
    results = []
...
def plot_perplexity(results: Dict[int, Dict], output_dir: Path, max_iter: int):
    ...
    plt.plot(x_smooth, y_smooth, '--', label=f'Полиномиальная аппроксимация (степень {best_degree}, R²={best_r2:.4f})', linewidth=2)
...
def main():
    ...
    parser.add_argument('--train_corpus', type=str, default='../../lab2/assets/annotated-corpus/train',
                        help='Путь к обучающему корпусу (аннотированному)')
    parser.add_argument('--test_dataset', type=str, default='../../lab2/assets/source-corpus/test.csv',
                        help='Путь к тестовому датасету (CSV)')
    parser.add_argument('--output', type=str, default='../assets',
                        help='Директория для сохранения результатов')
    parser.add_argument('--n_classes', type=int, default=4,
                        help='Количество классов в датасете')
    parser.add_argument('--max_iter', type=int, default=10,
                        help='Количество итераций обучения LDA')
    parser.add_argument('--experiment_iterations', action='store_true',
                        help='Провести эксперименты с разным количеством итераций')
    args = parser.parse_args()
    
    script_dir = Path(__file__).parent.resolve()
    
    train_corpus_dir = (script_dir / args.train_corpus).resolve() if not Path(args.train_corpus).is_absolute() else Path(args.train_corpus)
    test_dataset_path = (script_dir / args.test_dataset).resolve() if not Path(args.test_dataset).is_absolute() else Path(args.test_dataset)
