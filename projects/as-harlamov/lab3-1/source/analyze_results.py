...
def plot_svm_mlp_comparison(svm_results: Dict, mlp_results: List[Dict], output_dir: Path) -> None:
    ...
    ax3.plot(
        svm_iters, svm_f1s, marker='o', label=f'SVM ({svm_kernel})',
        color='#3498db', linewidth=2, markersize=8,
    )
    ax3.plot(
        mlp_iters, mlp_f1s, marker='s', label='MLP',
...
def plot_dimensionality_analysis(dimension_results: Dict, output_dir: Path) -> None:
    ...
    if 'dropout' in dimension_results:
        ...
        ax1_twin = ax1.twinx()
        line1 = ax1.plot(
            num_dropped, f1_scores, marker='o', color='#2ecc71',
            label='F1-score', linewidth=2, markersize=8,
        )
        line2 = ax1_twin.plot(
            num_dropped, accuracies, marker='s', color='#3498db',
    ...
    if 'pca' in dimension_results:
        ...
        ax2_twin = ax2.twinx()
        line1 = ax2.plot(
            n_components, f1_scores, marker='o', color='#2ecc71',
            label='F1-score', linewidth=2, markersize=8,
        )
        line2 = ax2_twin.plot(
            n_components, accuracies, marker='s', color='#3498db',
            label='Accuracy', linewidth=2, markersize=8,
        )
...
def analyze_dimensionality(dimension_results: Dict, output_dir: Path = None) -> None:
    ...
    if 'dropout' in dimension_results:
        ...
        for item in dropout_data[1:]:
            ...
            print(
                f"  Отброшено {item['Отброшено']} размерностей ({item['Осталось']} осталось): F1={f1_change:+.4f}, Acc={acc_change:+.4f}",
            )
    ...
    if 'pca' in dimension_results:
        ...
        for item in pca_data[1:]:
            ...
            acc_change = item_acc - baseline_acc
            print(
                f"  PCA до {item['Компоненты']} компонент ({item['Объясненная дисперсия']}): F1={f1_change:+.4f}, Acc={acc_change:+.4f}",
            )
    ...
    if 'dimension_experiments' in results:
        ...
        if 'dropout' in dim_exp and len(dim_exp['dropout']) > 0:
            ...
            if worst_dropout['num_dropped'] > 0:
                conclusions.append(
                    f"5. Отбрасывание размерностей: при отбрасывании {worst_dropout['num_dropped']} размерностей F1-score изменился с {baseline_f1:.4f} до {worst_dropout['f1_score']:.4f}",
                )
        ...
        if 'pca' in dim_exp and len(dim_exp['pca']) > 0:
            ...
            if best_pca['n_components'] != 100:
                conclusions.append(
                    f"6. PCA: оптимальное количество компонент - {best_pca['n_components']} (F1-score: {best_pca['f1_score']:.4f})",
                )
...
def main():
    ...
    parser.add_argument(
        '--results', type=str, default='../assets/results.json',
        help='Путь к файлу с результатами',
    )
    parser.add_argument(
        '--output_dir', type=str, default='../assets/plots',
