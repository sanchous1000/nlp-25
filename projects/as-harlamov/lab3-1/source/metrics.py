"""
Модуль для расчета метрик классификации вручную (без использования библиотечных методов).
"""
import numpy as np
...
def calculate_confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray, num_classes: int) -> np.ndarray:
    ...
    Args:
        y_true: Истинные метки классов (должны быть в диапазоне [0, num_classes-1])
        y_pred: Предсказанные метки классов (должны быть в диапазоне [0, num_classes-1])
        num_classes: Количество классов
    
    Returns:
        Матрица размерности (num_classes, num_classes), где элемент [i, j]
        означает количество объектов класса i, классифицированных как класс j.
    """
    cm = np.zeros((num_classes, num_classes), dtype=int)
    for true_label, pred_label in zip(y_true, y_pred):
        # Проверяем, что метки в допустимом диапазоне
        if 0 <= true_label < num_classes and 0 <= pred_label < num_classes:
            cm[int(true_label)][int(pred_label)] += 1
        else:
            # Это не должно происходить, но на всякий случай логируем проблему
            raise ValueError(
...
def calculate_precision(y_true: np.ndarray, y_pred: np.ndarray, num_classes: int) -> Dict[int, float]:
    ...
    for i in range(num_classes):
        ...
        fp = np.sum(cm[:, i]) - tp  # False Positives (все предсказания класса i, кроме правильных)
...
def calculate_recall(y_true: np.ndarray, y_pred: np.ndarray, num_classes: int) -> Dict[int, float]:
    ...
    for i in range(num_classes):
        tp = cm[i, i]  # True Positives
        fn = np.sum(cm[i, :]) - tp  # False Negatives (все объекты класса i, классифицированные неправильно)
...
def calculate_accuracy(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    ...
    Accuracy = (количество правильных предсказаний) / (общее количество образцов)
    """
...
def calculate_macro_averages(
    ...
    """
    Вычисляет макро-усредненные метрики.
...
def calculate_all_metrics(y_true: np.ndarray, y_pred: np.ndarray, num_classes: int) -> Dict:
    """
    Вычисляет все метрики и возвращает их в виде словаря.
    """
