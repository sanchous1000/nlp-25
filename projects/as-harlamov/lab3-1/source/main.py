...
from experiments import run_iteration_experiments, run_experiment
...
def main():
    ...
    parser.add_argument(
        '--lab2_path', type=str, default='../lab2',
        help='Путь к директории lab2',
    )
    parser.add_argument(
        '--generate_vectors', action='store_true',
        help='Сгенерировать векторы для train/test датасетов',
    )
    parser.add_argument(
        '--skip_basic', action='store_true',
        help='Пропустить базовые эксперименты',
    )
    parser.add_argument(
        '--skip_dimension', action='store_true',
